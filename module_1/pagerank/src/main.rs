...
that uses the hyperlink structure of the web to determine a quality ranking
for each web page. It works by counting the number and quality of links to a
page to determine a rough estimate of how important the website is.

Here we implement a simple example with a graph of major sports league
websites and their links. We add a dampening factor to ensure each page has
a minimum probability of being visited. This is a common technique used in PageRank.
The higher the rank, the higher website will appear in searches.
*/
